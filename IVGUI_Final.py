...
def sweep_reverse_bias():
    ...
    while True:
        ...
        try:
            ...
            I=abs((voltage-current)/ShuntReverse)*1000000
            voltages.append(voltage)
            currents.append(I)
            print(f"[INFO] Received V={voltage:.3f}V, current={I:.3f}µA")
        except ValueError:
            print(f"[ERROR] Unexpected data format: {line}")

    print("[INFO] All data received. Plotting...")
    update_graph("R")  # Plot after receiving all data
...
def sweep_forward_bias():
    ...
    while True:
        ...
        try:
            voltage, current = map(float, line.split(","))  #20,2*
            I=abs((voltage-current)/ShuntForward)*1000
            voltages.append(voltage)
            currents.append(I)
            print(f"[INFO] Received V={voltage:.3f}V, I={current:.3f}mA")
        except ValueError:
            print(f"[ERROR] Unexpected data format: {line}")

    print("[INFO] All data received. Plotting...")
    update_graph("F")  # Plot after receiving all data
    ...
    try:
        ...
        if line:
            try:
                ...
                voltages.append(voltage)
                I=abs((voltage-current)/ShuntForward)*1000
                print(f"[INFO] Received instant V={voltage:.2f}V, I={I:.2f}mA")
                currents.append(I)
                update_graph("F")
            except ValueError:
                print(f"[ERROR] Unexpected data format: {line}")
...
# Tkinter GUI
root = tk.Tk()
root.title("photocurrent Diode Measuring System")
root.geometry("1024x768")  # Adjusted for better aspect ratio

# Title Label
tk.Label(root, text="photocurrent Diode Measuring System", font=("Arial", 16, "bold")).pack(pady=10)
...
# Function to send a single character to Arduino
def send_command(command):
    if ser and ser.is_open:
        ser.write(command.encode())  # Send character command
        print(f"[INFO] Sent: {command}")  
    else:
        print("[ERROR] Not connected to Arduino!")

# Button functions
def red_led():
    current_color = red_led_button.cget("bg")
    new_color = "green" if current_color == "red" else "red"
    red_led_button.config(bg=new_color)
    send_command("0" if new_color == "green" else "1")  # '0' for ON, '1' for OFF

def green_led():
    current_color = green_led_button.cget("bg")
    new_color = "green" if current_color == "red" else "red"
    green_led_button.config(bg=new_color)
    send_command("2" if new_color == "green" else "3")  # '2' for ON, '3' for OFF

def blue_led():
    current_color = blue_led_button.cget("bg")
    new_color = "blue" if current_color == "red" else "red"
    blue_led_button.config(bg=new_color)
    send_command("4" if new_color == "blue" else "5")  # '4' for ON, '5' for OFF

def ir_led():
    current_color = ir_led_button.cget("bg")
    new_color = "purple" if current_color == "red" else "red"
    ir_led_button.config(bg=new_color)
    send_command("6" if new_color == "purple" else "7")  # '6' for ON, '7' for OFF

def uv_led():
    current_color = uv_led_button.cget("bg")
    new_color = "violet" if current_color == "red" else "red"
    uv_led_button.config(bg=new_color)
    send_command("8" if new_color == "violet" else "9")  # '8' for ON, '9' for OFF

def all_led():
    current_color = all_led_button.cget("bg")
    new_color = "green" if current_color == "red" else "red"
    all_led_button.config(bg=new_color)
    send_command("B" if new_color == "green" else "A")  # 'B' for ON, 'A' for OFF
...
red_led_button = tk.Button(led_frame, text="RED LED", command=red_led, bg="red", font=("Arial", 12, "bold"), width=10)
red_led_button.pack(side=tk.LEFT, padx=5)

green_led_button = tk.Button(led_frame, text="Green LED", command=green_led, bg="red", font=("Arial", 12, "bold"), width=10)
green_led_button.pack(side=tk.LEFT, padx=5)

blue_led_button = tk.Button(led_frame, text="Blue LED", command=blue_led, bg="red", font=("Arial", 12, "bold"), width=10)
blue_led_button.pack(side=tk.LEFT, padx=5)

ir_led_button = tk.Button(led_frame, text="IR LED", command=ir_led, bg="red", font=("Arial", 12, "bold"), width=10)
ir_led_button.pack(side=tk.LEFT, padx=5)

uv_led_button = tk.Button(led_frame, text="UV LED", command=uv_led, bg="red", font=("Arial", 12, "bold"), width=10)
uv_led_button.pack(side=tk.LEFT, padx=5)

all_led_button = tk.Button(led_frame, text="ALL", command=all_led, bg="red", font=("Arial", 12, "bold"), width=10)
all_led_button.pack(side=tk.LEFT, padx=5)
